...
from pickgames.tetris import mode as mode
...
class Tetris:
    ...
    def __init__(self, height, width, field, x, y):
        self.height = height
        self.width = width
        self.field = field
        self.x = x
    ...
    def draw_next_block(self, x):
        sx = topLeft_x + gameWidth + 20 + x
    ...
    def draw_hold_block(self, x):
        if self.holdBlock is not None:
            sx = topLeft_x + gameWidth - 300 + x
            sy = topLeft_y + gameHeight / 2 - 680
            self.holdBlock.y = 1
            for i in range(4):
                for j in range(4):
...
def startGame(level, difficulties, name):
    ...
    fps = 30
    game = Tetris(20, 10, [], 100, 60)
    twoplayer = Tetris(20, 10, [], 600, 60)
    counter = 0
    game.level = level
    twoplayer.level = level
    pressing_down = False
    ...
    while not game_over:
        ...
        if game.nextBlock is None:
            game.next_block()
        if twoplayer.block is None:
            twoplayer.new_block()
        if twoplayer.nextBlock is None:
            twoplayer.next_block()
        ...
        if counter % (fps // game.level // 2) == 0 or pressing_down:
            if game.state == "start":
                game.moveDown()
        if counter % (fps // twoplayer.level // 2) == 0 or pressing_down:
            if twoplayer.state == "start":
                twoplayer.moveDown()
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_LEFT:
                    twoplayer.moveHoriz(-1)
                if event.key == pygame.K_RIGHT:
                    twoplayer.moveHoriz(1)
                if event.key == pygame.K_KP_0:
                    twoplayer.moveBottom()
                if event.key == pygame.K_RSHIFT:
                    if twoplayer.holdBlock is None:
                        twoplayer.holdBlock = twoplayer.block
                        twoplayer.block = twoplayer.nextBlock
                        twoplayer.next_block()
                    else:
                        switch_block = twoplayer.holdBlock
                        twoplayer.holdBlock = twoplayer.block
                        twoplayer.block = switch_block
        ...
        # Updating the game board regularly for player one
        ...
        # Updating the game board regularly for player two
        for i in range(twoplayer.height):
            for j in range(twoplayer.width):
                pygame.draw.rect(screen, '#FFFFFF',
                                 [twoplayer.x + twoplayer.zoom * j, twoplayer.y + twoplayer.zoom * i, twoplayer.zoom,
                                  twoplayer.zoom], 1)
                if twoplayer.field [i] [j] > 0:
                    pygame.draw.rect(screen, shapeColors [twoplayer.field [i] [j]],
                                     [twoplayer.x + twoplayer.zoom * j + 1, twoplayer.y + twoplayer.zoom * i + 1,
                                      twoplayer.zoom - 2,
                                      twoplayer.zoom - 1])
        # Updating the board with the moving block for Player Two
        if twoplayer.block is not None:
            for i in range(4):
                for j in range(4):
                    p = i * 4 + j
                    if p in twoplayer.block.image():
                        pygame.draw.rect(screen, shapeColors [twoplayer.block.color],
                                         [twoplayer.x + twoplayer.zoom * (j + twoplayer.block.x) + 1,
                                          twoplayer.y + twoplayer.zoom * (i + twoplayer.block.y) + 1,
                                          twoplayer.zoom - 2, twoplayer.zoom - 2])
        ...
        displayText("Player 1 Controls", 100, 500, 20)
        displayText("Rotate CLockWise: W", 100, 520, 20)
        displayText("Left: A", 100, 580, 20)
        displayText("Right: D", 100, 600, 20)
        displayText("Soft Drop: S", 100, 540, 20)
        displayText("Hard Drop: Space", 100, 560, 20)
        displayText("Hold : LeftShift", 100, 620, 20)

        # Showing the score for Player Two
        displayText("Score: " + str(game.score), 800, 0, 20)
        displayText("Next Shape ", 800, 50, 20)
        displayText("Hold ", 520, 50, 20)
        displayText("Player 2 Controls", 600, 500, 20)
        displayText("Rotate CLockWise: ARROW UP", 600, 520, 20)
        displayText("Left: ARROW LEFT", 600, 580, 20)
        displayText("Right: ARROW RIGHT", 600, 600, 20)
        displayText("Soft Drop: ARROW DOWN", 600, 540, 20)
        displayText("Hard Drop: NUMPAD 0", 600, 560, 20)
        displayText("Hold : RIGHT SHIFT", 600, 620, 20)

        # Ending the game if state is Game Over
        game.draw_ghost_block()
        game.draw_hold_block(0)
        game.draw_next_block(0)

        twoplayer.draw_ghost_block()
        twoplayer.draw_hold_block(500)
        twoplayer.draw_next_block(500)

        if game.state == "gameover":
            screen.fill("#808080")
            displayText("Player 2 Wins", 60, 250, 50)
            displayText("Press ESC / Q to Quit or Enter to Play Again", 60, 360, 50)
            while game_close:
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        pygame.quit()
                    ...
                    if event.type == pygame.KEYDOWN:
                        if event.key == pygame.K_q:
                            ...
                            startGame(level, difficulties, name)
                pygame.display.update()
        if twoplayer.state == "gameover":
            screen.fill("#808080")
            displayText("Player 1 Wins", 60, 250, 50)
            displayText("Press ESC / Q to Quit or Enter to Play Again", 60, 360, 50)
            while game_close:
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
